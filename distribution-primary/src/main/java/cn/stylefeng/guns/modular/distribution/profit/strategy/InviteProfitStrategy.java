...
public class InviteProfitStrategy extends AbstractProfit {
    ...
    public void validateBiz(ProfitAccess subject) throws DistributionException {
        log.debug("【邀请分润】 ---> 验证业务合法性 开始");
        DistMemberDTO newJoinMemberParam = (DistMemberDTO) subject;
        log.debug("【邀请分润】 ---> 验证业务合法性 结束");
    }

    @Override
    public DistProfitEvent recordEvent(ProfitAccess subject) {
        log.debug("【邀请分润】 ---> 记录分润事件 开始");
        // 记录发生了邀请分润事件
        DistMemberDTO newJoinMemberParam = (DistMemberDTO) subject;
        ProfitTypeEnum eventEnum = ProfitTypeEnum.INVITE;
        DistProfitEvent event = new DistProfitEvent()
                .setProfitType(eventEnum)
                .setEventNumber(eventEnum.generateEventNumber())
                .setTriggerMemberUsername(newJoinMemberParam.getMemberUsername());
        profitEventService.save(event);
        log.debug("【邀请分润】 ---> 记录分润事件 结束");
        return event;
    }
    ...
    public void executeProfit(ProfitAccess subject, DistProfitEvent event) {
        log.debug("【邀请分润】 ---> 执行分润 开始 .....");
        // 执行真正的分润
        DistMemberDTO newJoinMemberParam = (DistMemberDTO) subject;
        // 组装分润数组。邀请分润,parentPath肯定不为空
        String profitPath = newJoinMemberParam.getMemberUsername() + "." + newJoinMemberParam.getParentPath();
        String[] pathArray = profitPath.split("\\.");
        log.debug("【邀请分润】 ---> 当前分润人员标识为：[{}]", Arrays.toString(pathArray));
        ...
        for (int level = 0; level < pathArray.length; level++) {
            ...
            // 查询佣金和积分的分润参数
            DistProfitParam moneyParam = this.getProfitParam(AccountTypeEnum.MONEY, ProfitTypeEnum.INVITE, levelEnum, member.getMemberType(), member.getMemberRank());
            DistProfitParam integralParam = this.getProfitParam(AccountTypeEnum.INTEGRAL, ProfitTypeEnum.INVITE, levelEnum, member.getMemberType(), member.getMemberRank());
