...
@Component
@Slf4j
public class TradeProfitStrategy extends AbstractProfit {

    @Override
    public void validateBiz(ProfitAccess subject) throws DistributionException {
        log.debug("【商品分润】 ---> 验证业务合法性 开始");
        DistProfitEventDTO distProfitEventDTO = (DistProfitEventDTO) subject;
        AssertHelper.isTrue(memberService.exists(distProfitEventDTO.getTriggerMemberUsername()), DistributionException.BizEnum.MEMBER_NOT_EXIST);
        log.debug("【商品分润】 ---> 验证业务合法性 结束");
    }

    @Override
    public DistProfitEvent recordEvent(ProfitAccess subject) {
        log.debug("【商品分润】 ---> 记录分润事件 开始");
        // 记录发生了邀请分润事件
        DistProfitEventDTO distProfitEventDTO = (DistProfitEventDTO) subject;
        ProfitTypeEnum eventEnum = ProfitTypeEnum.TRADE;
        DistProfitEvent event = new DistProfitEvent()
                .setProfitType(eventEnum)
                .setEventNumber(distProfitEventDTO.getEventNumber())
                .setEventPrice(distProfitEventDTO.getEventPrice())
                .setTriggerMemberUsername(distProfitEventDTO.getTriggerMemberUsername());
        profitEventService.save(event);
        log.debug("【商品分润】 ---> 记录分润事件 结束");
        return event;
    }

    @Override
    public void executeProfit(ProfitAccess subject, DistProfitEvent event) {
        log.debug("【商品分润】 ---> 执行分润 开始 .....");
        // 执行真正的分润
        DistProfitEventDTO distProfitEventDTO = (DistProfitEventDTO) subject;
        // 组装分润数组,如果当前触发人员无父级，变成数组时，末尾的【.】也不会影响数组长度
        String profitPath = distProfitEventDTO.getTriggerMemberUsername() + "." + memberService.findByMemberUsername(distProfitEventDTO.getTriggerMemberUsername()).getParentPath();

        String[] pathArray = profitPath.split("\\.");
        log.debug("【商品分润】 ---> 当前分润人员标识为：[{}]", Arrays.toString(pathArray));

        // 记录当前能获得分润的人员
        List<String> profitMembers = CollUtil.newLinkedList();
        ...
        for (int level = 0; level < pathArray.length; level++) {
            ...
            // 查询佣金和积分的分润参数
            DistProfitParam moneyParam = this.getProfitParam(AccountTypeEnum.MONEY, ProfitTypeEnum.TRADE, levelEnum, member.getMemberType(), member.getMemberRank());
            DistProfitParam integralParam = this.getProfitParam(AccountTypeEnum.INTEGRAL, ProfitTypeEnum.TRADE, levelEnum, member.getMemberType(), member.getMemberRank());
